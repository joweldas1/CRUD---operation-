...
const UpdateCoffee = () => {
    ...
    const {name,supplier,category,chef,taste,details,image,price}=coffee
    console.log(coffee);
    const handleToAddCoffee=e=>{
        ...
        // send data to server
        fetch(`https://coffee-store-server-fawn-mu.vercel.app/coffee/${coffee._id}`,{
            method:"PUT",
        ...
        .then(data=>{
            if(data){
                Swal.fire({
                    title: 'Update!',
                    text: 'Coffee Details Update done',
                    icon: 'success',
    ...
    return (
      ...
      lg:border-4 filter  border-blue-400">
        <h1 className="font-rancho text-center text-3xl font-semibold filter blur-[0.2px]">Update  Coffee : {coffee.name}</h1>
